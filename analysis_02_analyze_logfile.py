...
for i in range(len(blockRangeRequests)):
    ...
    ax.bar(i, time_to_find_sync_peers[i], bottom=bottoms[i], label='Time to find sync peers' if i == 0 else "", color=sync_peers_color)
    ...
    for construct_time in times_to_construct_requests[i]:
        ax.bar(i, construct_time, bottom=bottoms[i], label='Time to construct requests' if i == 0 and construct_time == times_to_construct_requests[i][0] else "", color=construct_requests_color)
        bottoms[i] += construct_time

    # Plot time to construct combined request (single value)
    ax.bar(i, time_to_construct_combined_request[i], bottom=bottoms[i], label='Time to construct combined request' if i == 0 else "", color=construct_combined_color)
    bottoms[i] += time_to_construct_combined_request[i]

    # Plot time to send request (single value)
    ax.bar(i, time_to_sent_request[i], bottom=bottoms[i], label='Time to send request' if i == 0 else "", color=sent_request_color)
    bottoms[i] += time_to_sent_request[i]

    # Plot time to receive response (single value)
    ax.bar(i, time_to_received_response[i], bottom=bottoms[i], label='Time to receive response' if i == 0 else "", color=received_response_color)
    bottoms[i] += time_to_received_response[i]

    # Plot time to deserialize response (single value)
    ax.bar(i, time_to_deserialized[i], bottom=bottoms[i], label='Time to deserialize' if i == 0 else "", color=deserialized_color)
    ...
    for check_time, extra_time in zip(times_check_next_block_done[i], times_extra_message_advanced_to_block[i]):
        ax.bar(i, check_time, bottom=bottoms[i], label='Check next block time' if i == 0 and check_time == times_check_next_block_done[i][0] else "", color=check_next_color)
        bottoms[i] += check_time

        ax.bar(i, extra_time, bottom=bottoms[i], label='Extra message advanced to block' if i == 0 and extra_time == times_extra_message_advanced_to_block[i][0] else "", color=extra_message_advanced_color)
        bottoms[i] += extra_time

    # Plot unaccounted time (single value)
    ax.bar(i, unaccounted_times[i], bottom=bottoms[i], label='Unaccounted time' if i == 0 else "", color=unaccounted_color)
    bottoms[i] += unaccounted_times[i]

# Add labels and title
ax.set_xlabel('Block range request')
