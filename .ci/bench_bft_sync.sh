...
$TASKSET1 stdbuf -oL -eL snarkos start --dev 0 --validator "${common_flags[@]}" \
  --logfile="$log_dir/validator-0.log" > >(awk -v prefix="[validator-0] " '{print prefix $0}') 2>&1 &
PIDS[0]=$!
...
for node_index in $(seq 1 "$num_nodes"); do
  ...
  $TASKSET2 stdbuf -oL -eL snarkos start "--dev=$node_index" --validator \
    "${common_flags[@]}" "--validators=$validators" \
    "--logfile=$log_dir/$name.log" > >(awk -v prefix="[$name] " '{print prefix $0}') 2>&1 &
  PIDS[node_index]=$!
