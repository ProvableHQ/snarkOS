...
for ((validator_index = 0; validator_index < $total_validators; validator_index++)); do
  ...
  if [ "$validator_index" -eq 0 ]; then
    snarkos start --nodisplay --network $network_id --dev $validator_index --allow-external-peers --dev-num-validators $total_validators --validator --logfile $log_file --metrics --no-dev-txs --dev-proposal-delay 100 &
  else
    snarkos start --nodisplay --network $network_id --dev $validator_index --allow-external-peers --dev-num-validators $total_validators --validator --logfile $log_file --dev-proposal-delay 100 &
  fi
