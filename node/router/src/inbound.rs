    ...
    async fn inbound(&self, peer_addr: SocketAddr, message: Message<N>) -> Result<bool> {
        ...
        match message {
            ...
            Message::BlockResponse(BlockResponse { request, latest_consensus_version, blocks, .. }) => {
                ...
                let node = self.clone();
                spawn_blocking(move || node.block_response(peer_ip, blocks.0, latest_consensus_version))
                    .await?
            ...
            Message::Ping(message) => {
                ...
                // If the peer is a prover, ensure there are no block locators.
                    ...
                    bail!("Peer '{peer_ip}' is a prover, but a block height was provided");
                }

                // Process the ping message.
                self.ping(peer_ip, message).await.with_context(|| format!("Peer '{peer_ip}' sent an invalid ping"))?;
                Ok(true)
            }
            Message::Pong(message) => {
                self.pong(peer_ip, message).with_context(|| format!("Peer '{peer_ip}' sent an invalid pong"))?;
                Ok(true)
            }
