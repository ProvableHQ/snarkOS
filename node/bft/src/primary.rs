...
pub struct Primary<N: Network> {
    ...
    primary_callback: Arc<RwLock<Option<Arc<dyn PrimaryCallback<N>>>>>,
...
impl<N: Network> Primary<N> {
    ...
    pub async fn run(
        ...
        if let Some(callback) = primary_callback {
            let prev = self.primary_callback.write().replace(callback);
            ensure!(prev.is_none(), "Primary callback already set");
        }
...
impl<N: Network> Primary<N> {
    ...
    pub async fn propose_batch(&self) -> Result<()> {
        ...
        if self.storage.contains_certificate_in_round_from(round, self.gateway.account().address()) {
            ...
            if let Some(cb) = self.primary_callback.read().as_ref() {
...
impl<N: Network> Primary<N> {
    ...
    async fn try_increment_to_the_next_round(&self, next_round: u64) -> Result<()> {
        ...
        if current_round < next_round {
            ...
            let is_ready = if let Some(cb) = self.primary_callback.read().clone() {
    ...
    /// Get the `PrimaryCallback` if one is set.
    fn get_callback(&self) -> Option<Arc<dyn PrimaryCallback<N>>> {
        self.primary_callback.read().clone()
    }
