...
use crate::{
    ...
    helpers::{Cache, PrimarySender, Resolver, Storage, WorkerSender, assign_to_worker},
};
...
impl<N: Network> Gateway<N> {
    ...
    pub async fn run(
        ...
        if let Some(sync_callback) = sync_callback {
            if self.sync_callback.write().replace(sync_callback).is_some() {
                error!("Sync callback already set in gateway");
            }
        }
...
impl<N: Network> Gateway<N> {
    ...
    fn remove_connected_peer(&self, peer_ip: SocketAddr) {
        ...
        if let Some(cb) = &*self.sync_callback.read() {
    ...
    async fn inbound(&self, peer_addr: SocketAddr, event: Event<N>) -> Result<()> {
        ...
        match event {
            ...
            Event::BlockResponse(block_response) => {
                ...
                let sync_callback = (*self.sync_callback.read()).clone();
                if let Some(cb) = sync_callback {
