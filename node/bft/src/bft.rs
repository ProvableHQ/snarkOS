...
impl<N: Network> BFT<N> {
    ...
    async fn update_dag<const ALLOW_LEDGER_ACCESS: bool, const IS_SYNCING: bool>(
        ...
        info!("Proceeding to commit round {commit_round} with leader '{}' (syncing={IS_SYNCING})", fmt_id(leader));
            ...
            if !IS_SYNCING {
                ...
                if let Some(consensus_sender) = self.consensus_sender.get() {
                    ...
                    match callback_receiver.await {
                        ...
                        Err(err) => {
                            err.log_error("BFT failed to receive the callback for round {anchor_round}");
                            return Ok(());
                        }
                    }
                }
