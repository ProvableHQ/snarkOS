...
impl<N: Network> Sync<N> {
    ...
    async fn sync_storage_with_ledger_at_bootup(&self) -> Result<()> {
        ...
        if let Some(cb) = self.sync_callback.get() {
            cb.sync_dag_at_bootup(certificates).with_context(|| "Failed to update the DAG from sync")?;
        }
    ...
    async fn add_block_subdag_to_bft(&self, block: &Block<N>) -> Result<()> {
        ...
        for certificates in subdag.values().cloned() {
            ...
            if let Some(callback) = self.sync_callback.get() {
    ...
    fn try_sync_storage_with_block(&self, new_block: &Block<N>) -> Result<bool> {
        ...
        while let Some(pending_block) = pending_blocks.front()
            && pending_block.height() <= ledger_block_height
        {
            trace!(
...
mod tests {
    ...
    async fn test_commit_chain() -> anyhow::Result<()> {
        ...
        for block_height in 1..=num_blocks {
            ...
            let block = task::spawn_blocking(move || {
                let ledger_update = core_ledger.begin_ledger_update().unwrap();
                let block = ledger_update.prepare_advance_to_next_quorum_block(subdag, Default::default())?;
                ledger_update.advance_to_next_block(&block)?;
                Result::<Block<CurrentNetwork>>::Ok(block)
            })
            .await?;
    ...
    async fn test_pending_certificates() -> anyhow::Result<()> {
        ...
        task::spawn_blocking(move || {
            // Create block 1.
            let update1 = core_ledger_cpy.begin_ledger_update().unwrap();
        ...
        let core_ledger_cpy = core_ledger.clone();
        task::spawn_blocking(move || {
            let update2 = core_ledger_cpy.begin_ledger_update().unwrap();
