...
impl<N: Network> Sync<N> {
    ...
    #[cfg(test)]
    pub async fn test_update_peer_locators(&self, peer_ip: SocketAddr, locators: BlockLocators<N>) -> Result<()> {
...
impl<N: Network> Sync<N> {
    ...
    async fn try_advancing_block_synchronization(&self) -> Result<bool> {
        ...
        let _tip = self.block_sync.get_peer_heights().await.values().max().copied().unwrap_or(0);
