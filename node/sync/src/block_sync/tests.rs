...
async fn test_update_peer_locators() {
    ...
    for peer1_height in 0..500u32 {
        ...
        .await
        .unwrap();
        assert_eq!(sync.get_peer_height(&peer1_ip).await, Some(peer1_height));
        ...
        for peer2_height in 0..500u32 {
            ...
            .await
            .unwrap();
            assert_eq!(sync.get_peer_height(&peer2_ip).await, Some(peer2_height));
        }
    }
}
...
async fn test_remove_peer() {
    ...
    assert_eq!(sync.get_peer_height(&peer_ip).await, Some(100));

    sync.remove_peer(&peer_ip);
    assert_eq!(sync.get_peer_height(&peer_ip).await, None);

    sync.update_peer_block_locators(peer_ip, sample_block_locators(0, 200)).await.unwrap();
    assert_eq!(sync.get_peer_height(&peer_ip).await, Some(200));
