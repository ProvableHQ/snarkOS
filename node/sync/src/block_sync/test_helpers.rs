...
pub async fn check_prepare_block_requests<C: CommunicationService>(
    ...
    let (requests, _sync_peers) = sync.prepare_block_requests(communication).await;
    ...
    let expected_num_requests = (min_common_ancestor as usize).min(MAX_BLOCK_REQUESTS);
    assert_eq!(requests.len(), expected_num_requests);

    for (idx, (height, (hash, previous_hash, _num_sync_ips))) in requests.into_iter().enumerate() {
