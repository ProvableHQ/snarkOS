...
use snarkos_node::{
    Node,
    router::{NodeClass, Peer, PeerPoolHandling, messages::NodeType},
    tcp::P2P,
};
...
impl Overview {
    ...
    fn draw_peer_table<N: Network>(
        ...
        let mut peers = node.router().get_peers();
        ...
        let rows: Vec<_> = peers
            ...
            .map(|peer| {
                ...
                let node_type =
                    peer.node_type().map(|ntype| ntype.to_string()).unwrap_or_else(|| "unknown".to_string());
                ...
                let (download_speed, upload_speed, download_total, upload_total) =
                    if let Some(stats) = node.router().tcp().known_peers().get(peer.listener_addr().ip()) {
                        let now = Instant::now();
