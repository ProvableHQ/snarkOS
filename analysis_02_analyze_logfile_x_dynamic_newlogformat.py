...
log_file_path = os.path.join(os.getcwd(), "aws-logs23", log_file_name)
...
class BlockRequest:
    def __init__(self, time_constructed, height, raw_time_to_construct):
        ...
        self.time_starting_check_next_block = None
    ...
    def get_time_to_check_next_block_done(self):
        if self.corresponding_block_check_next_block_time is None:
            ...
            return pd.Timedelta(seconds=0)
        if self.time_starting_check_next_block is None:
            return pd.Timedelta(seconds=0)
        return self.corresponding_block_check_next_block_time - self.time_starting_check_next_block
...
def get_blockRangeRequest_by_height(currentTryBlockSyncCall, height):
    if currentTryBlockSyncCall is not None:
        if height in currentTryBlockSyncCall.BlockRangeRequests:
            return currentTryBlockSyncCall.BlockRangeRequests[height]
    for tryBlockSyncCall in tryBlockSyncCalls:
        for blockRangeRequest in tryBlockSyncCall.BlockRangeRequests.values():
            if(blockRangeRequest.start_height == height):
...
for index, row in event_df.iterrows():
    ...
    if "SYNCPROFILING Processing block response for height" in message or "SYNCPROFILING Received block response for start height" in message:
        ...
        get_blockRangeRequest_by_height(current_tryBlockSyncCall, height).time_received_response = timestamp
        continue
    ...
    if "SYNCPROFILING Deserialized blocks BlockRequest { start_height" in message:
        ...
        get_blockRangeRequest_by_height(current_tryBlockSyncCall, start_height).time_deserialized = timestamp
        continue
    ...
    if "SYNCPROFILING starting check next block in" in message:
        height = int(message.split('height ')[1].split(' ')[0])
        get_blockRequest_by_height(current_tryBlockSyncCall, height).time_starting_check_next_block = timestamp
        continue
    ...
    if "SYNCPROFILING CHECK NEXT BLOCK TIME" in message or "SYNCPROFILING Check next block in " in message:
    ...
    if "SYNCPROFILING ADVANCE TO NEXT BLOCK TIME" in message or "SYNCPROFILING Advance to next block in sync_ledger_with_block_without_bft took" in message:
